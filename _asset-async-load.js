...
(function(name, definition) {
    ...
    // Start now or when DOM is ready
    if (jQuery.isReady) {
        theModule.init();
    } else {
    ...
    // Ensure that jQuery is globally set to $
    if (!$ && jQuery) {
        window.$ = window.jQuery;
        if (console.info)
            console.info('AssetAsyncLoad set jQuery to window.$');
    }
        ...
        init: function() {
            ...
            if (module.$imageSrc.length > 0) {
                module.$imageSrc.each(function() {
                    ...
                    var elementAttribute = $imageElement.attr('data-asset-async-src-image');

                    // todo: if element is not img, convert it to an image
                    //if ($imageElement.prop('nodeName').toLowerCase() !== 'img') {
                    //
                    //    $imageElement.after("<img class='" + $imageElement.attr('class') + "' data-asset-async-src-image='" + elementAttribute + "' src='#'>");
                    //}

                    if (module.isValidImagesArray(elementAttribute)) {
                        module.applyResponsiveSource($imageElement, 'data-asset-async-src-image');
                        module.responsiveElementsCount++;
                    } else {
                        $imageElement.attr('src', elementAttribute);
                    }
